...
public class StoreKitEventListener : MonoBehaviour
{
	...
	public GameObject iapController;
#if UNITY_IPHONE
	void OnEnable()
	{
	...
	void purchaseFailed( string error )
	{
		...
		iapController.SendMessage("PurchaseFailed");
	...
	void purchaseCancelled( string error )
	{
		...
		iapController.SendMessage("PurchaseCancled");
	...
	{
		Debug.Log( "purchased product: " + transaction.productIdentifier +",quantity:" + transaction.quantity );
		iapController.SendMessage("PurchaseSucceed", transaction.productIdentifier);
	}
